...
for folder in setslist: 
	...
	for song in filelist: # filelist[:] makes a copy of filelist.
		if (song.endswith("mid.p")):
			song_array = pickle.load(open(songs_folder + '/' + song, "rb"))

			conjunct_melodic_motion.append(music_geometry_eval.calculate_conjunct_melodic_motion(song_array))
			limited_macroharmony.append(music_geometry_eval.calculate_limited_macroharmony(song_array, span_size_lm))
			centricity.append(music_geometry_eval.calculate_centricity(song_array, span_size_centr))
...
partial_result = result['db12']
print('&db12  & {0:.2f} $\\pm$ {1:.2f} & {2:.2f} $\\pm$ {3:.2f} & {4:.2f} $\\pm$ {5:.2f} \\\\').format(partial_result['lstm'][m3]['desc_stats'].mean, partial_result['lstm'][m3]['std_dev'], partial_result['nas'][m3]['desc_stats'].mean, partial_result['nas'][m3]['std_dev'], partial_result['ugrnn'][m3]['desc_stats'].mean, partial_result['ugrnn'][m3]['std_dev'])
print '\\hline'
